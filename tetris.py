...
def draw_grid(surface, row , col):
    ...
    for i in range(row):
        # horizontal
        pygame.draw.line(surface, (128,128,128), (sx,  sy + i * 30), (sx + gameWidth, sy + i * 30)) 
        for j in range(col):
            # vertical 
            pygame.draw.line(surface, (128,128,128), (sx + j * 30, sy), (sx + j * 30, sy + gameHeight))  
...
def draw_window(surface):
    ...
    surface.blit(label, (top_left_x + gameWidth / 2 - (label.get_width() / 2), 30))
...
"""
Job : checks whether the user has lost the game
Return :  bool 
"""
def check_lost(locked_position):
...
"""
Job : displays a message in the middle of my screen
Return :  None 
"""
def display_middle_text(text, size, color, surface):
    pygame.font.init()
    font = pygame.font.SysFont('comicsans', size, bold = True)
    label = font.render(text, 1, color)
    surface.blit(label, (top_left_x + gameWidth / 2 - (label.get_width() / 2)  , top_left_y + gameHeight / 2 - (label.get_height() / 2)))
...
def main():
    ...
    while run:
        ...
        # did user lose the game:
        if check_lost(locked_position):
            run = False


    display_middle_text("You Lost", 40, (255,255,255), win)
    pygame.display.update()
...
def main_menu():
    ...
    while run:
        win.fill((0,0,0))
        display_middle_text("Press any key to begin.", 60, (255,255,255), win)
        pygame.display.update()
...
pygame.display.set_caption("Tetris with pygame by Ardy")
main_menu()
